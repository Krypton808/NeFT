...
def main(
    ...
    for instruction in val_input:
        generate_input = f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>{sys_prompt}<|eot_id|><|start_header_id|>user<|end_header_id|>Instruction: {instruction}<|eot_id|><|start_header_id|>assistant<|end_header_id|>"
    ...
    for Instruction, data in zip(val_input, dataloader):
        ...
        for o in output:
            ...
            # output_str_list_count_eot = output_str.split("<|end_of_text|>")
            print(output_str.strip())
            print(len(output_str_list_count_eot))

            output_str = output_str.split("<|start_header_id|>assistant<|end_header_id|>")[1]
            output_str = output_str.replace("<|eot_id|>", "")
            # output_str = output_str.replace("<|end_of_text|>", "")
            output_str_list.append(output_str)
            print('*******************')
            writer.write({"prompt": Instruction, "response": output_str.strip()})
            i += 1
...
CUDA_VISIBLE_DEVICES=5 python infer_Llama-3.2-1B-Instruct.py \
    --model_path /mnt/data1/output/model/llama3_2_1B_alpaca_neuron_6000_2/checkpoint-6500 \
    --validate_file_path /mnt/data1/study/data/mmlu/validation.jsonl \
    --key_name 'instruction' \
    --save_path /mnt/data1/output/mmlu/llama3_2_1B_alpaca_neuron_6000_2/6500step/output.jsonl


CUDA_VISIBLE_DEVICES=0 python infer_Llama-3.2-1B-Instruct.py \
    --model_path /mnt/data1/output/model/llama3_2_1B_alpaca/checkpoint-6500_backup \
    --validate_file_path /mnt/data1/study/data/mmlu/validation.jsonl \
    --key_name 'instruction' \
    --save_path /mnt/data1/output/mmlu/llama3_2_1B_alpaca/6500step/output.jsonl
