...
def classifier_probe_generalization_experiment(activation_dict, probe_dict, target, is_test):
    train_target = target[~is_test]
    test_target = target[is_test]
    ...
    for prompt_a, activations_a in activation_dict.items():
        for prompt_b, (probe_b, bias_b) in probe_dict.items():
            ...
            train_scores = score_time_probe(
                train_target, a_proj_b[~is_test])
            test_scores = score_time_probe(
                test_target, a_proj_b[is_test])
...
if __name__ == "__main__":
    ...
    for prompt in args.prompt_names:
        ...
        probe_result = pickle.load(open(
            r"/data5/haoyun.xu/study/MI/MMMI/src/MI/experiment_setup/probes/results/Llama-2-7b-chat-hf/xnli/en/xnli.en.mean.{prompt}.p".format_map({"prompt": prompt}),
            'rb'))
    ...
    results = classifier_probe_generalization_experiment(activation_dict, probe_dict, target, is_test)
